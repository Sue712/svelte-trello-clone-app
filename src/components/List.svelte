<script>
    ...
    function disabledSortable(event) {
...
<div class="list">
    <div class="list__inner">
        <div class="list__heading">
            <ListTitle {list} on:editMode={disabledSortable} />
            <p>{list.cards.length} cards</p>
            <div data-list-id={list.id} class="list__cards" bind:this={cardsEl}>
                {#each list.cards as card (card.id)}
                    <Card
                        {card}
                        listId={list.id}
                        on:editMode={disabledSortable}
                    />
                {/each}
            </div>
            <CreateCard listId={list.id} on:editMode={disabledSortable} />
        </div>
    </div>
</div>
